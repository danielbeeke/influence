...
export const JsonLdProxy = (data, context, extraCommands: { [key: string]: (value) => any} = {}, defaultAliasses: Array<string> = []) => {
  ...
  return new Proxy(data, {
    get(target, prop, receiver) {
      ...
      if (prop === '_alias') return defaultAliasses
      ...
      if (defaultAliasses.length && !prop.toString().includes(':') && !Reflect.has({}, prop) && !Reflect.has([], prop)) {
        ...
        for (const defaultAlias of defaultAliasses) {
          ...
          if (isOurProperty && Reflect.has(target, newProp) && target[newProp]) {
